...
public class ChannelJoinRequestAdapter extends FirestoreAdapter<ChannelJoinRequestAdapter.ViewHolder> {

    public interface OnMyChannelSelectedListener {
        void onAccept(String memberId, String userId);
    }

    private OnMyChannelSelectedListener mListener;

    public ChannelJoinRequestAdapter(Query query, OnMyChannelSelectedListener listener) {
        super(query);
        mListener = listener;
    }
            ...
            docRef.get().addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    ...
                    if (document.exists()) {
                        ...
                        tvDateRequestChannelJoin.setText("Request on " + GetDateName(membersData.getMemberRequestDate().toString()));
                        if (url.isEmpty()) {
                        ...
                        btnChannelJoinRequestAccept.setOnClickListener(view -> {
                            if (listener != null) {
                                listener.onAccept(membersData.getMemberId(), user.getUserId());
                            }
                        });
                    }
                }
